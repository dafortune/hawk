...
describe("passport-hawk", function () {
  ...
  it("should call done with false when the id doesnt exist", function (testDone) {
    ...
    var authHeader = Hawk.client.header(
      "http://example.com:8080/resource/4?filter=a",
      "POST",
  ...
  it("should fail with a stale request", function (testDone) {
    var fixedHeader =
      'Hawk id="dasd123", ts="1366220539", nonce="xVO62D", mac="9x+7TGN6VLRH8zX5PpwewpIzvf+mTt8m7PDQQW2NU/U="';
    var req = buildRequest({
      headers: {
  ...
  it("can authenticate a request with options", function (testDone) {
    var header = Hawk.client.header(
      "https://example.com/resource/4?filter=a",
      "GET",
      { credentials: credentials }
    );
    var req = buildRequest({
